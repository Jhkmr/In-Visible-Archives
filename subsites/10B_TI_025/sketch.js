...
let modeBtn, imgToggleBtn, invertBtn;
let showImage = false;
...
function setup() {
  ...
  blurSlider     = createSlider(1, 21, 17, 2);
  threshSlider   = createSlider(0, 255, 181, 1);
  contrastSlider    = createSlider(0, 400, 100, 1);
  dotSpacingSlider    = createSlider(2, 100, 3, 1);
  activeDotSizeSlider = createSlider(1, 60, 42, 1);
  ...
  invertBtn = createButton('Invert: Off');
  invertBtn.mousePressed(() => {
    invertImage = !invertImage;
    invertBtn.html('Invert: ' + (invertImage ? 'On' : 'Off'));
    contoursDirty = true;
  });
...
function draw() {
  ...
  invertBtn.position(gridX + 8,             gy + 155);
  ...
  if (sourceImage) {
    ...
    if (layoutMode === 'base') {
      for (let r of detectedRegions) {
        ...
        if (invertImage) { fill(0); stroke(255); } else { noFill(); stroke(0, 0, 0); } strokeWeight(0);
        rectMode(CENTER); rect(0, 0, bw, bh);

        if (r.contours && r.contours.length > 0) {
          stroke(invertImage ? 255 : 0); strokeWeight(1); noFill();
...
function drawDotGrid(gw, gh, contours, cropW, cropH) {
  ...
  //EDIT HERE$$
  angleMode(DEGREES);stroke(invertImage ? 255 : 0); strokeWeight(1); fill(invertImage ? 0 : 255); //noFill();// //rectMode(CENTER);
  for (let yi = 0; yi < nr; yi++)
    for (let xi = 0; xi < nc; xi++)
      if (isOutline(xi, yi)) {
        //fill(random([0, 255])); noLoop();
        push();
        translate(gridX + xs[xi], gridY + ys[yi]);
        //shearX(45);
        //rotate(45);
        rect(0, 0, cellW*sin(yi), rows*cellH - (gridY + ys[yi])); //rect(0, 0, activeSize, activeSize);
        //line(0, 0, 0, rows*cellH - (gridY + ys[yi])); //line(0, 0, activeSize, activeSize);
        pop();
      }
  mask.remove();
}
...
function extractContours(pg, blurK, thresh) {
  ...
  cv.threshold(blurM, binM, thresh, 255, invertImage ? cv.THRESH_BINARY : cv.THRESH_BINARY_INV);
...
function saveImage() {
  ...
  if (layoutMode === 'base') {
    for (let r of detectedRegions) {
      ...
      if (r.contours && r.contours.length > 0) {
        pg.stroke(invertImage ? 255 : 0); pg.strokeWeight(1); pg.noFill();
        for (let pts of r.contours) {
          pg.beginShape();
    ...
    if (r && r.cropPg) {
      ...
      //EDIT HERE$$
      pg.stroke(invertImage ? 255 : 0); pg.strokeWeight(1); pg.angleMode(DEGREES); pg.fill(invertImage ? 0 : 255); //; //pg.rectMode(CENTER);
      for (let yi = 0; yi < snr; yi++)
        for (let xi = 0; xi < snc; xi++)
          if (sIsOutline(xi, yi)) {
            //pg.fill(random([0, 255])); pg.noLoop();
            pg.push();
            pg.translate(sxs[xi], sys[yi]);
            //pg.shearX(45);
            //pg.rotate(45);
            //pg.rect(0, 0, activeSize, gridY);
            pg.rect(0, 0, cellW*sin(yi), rows*cellH - sys[yi]);
            pg.pop();
          }
      maskBuf.remove();
    }
