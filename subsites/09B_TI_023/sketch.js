...
let btnLoad, btnClear, btnSave, btnAutomate;
...
function setup() {
  ...
  padSliderX     = createSlider(0, 1, 0.13, 0.01);
  padSliderY     = createSlider(0, 1, 0.45, 0.01);
  blurSlider     = createSlider(1, 21, 1, 2);
  threshSlider   = createSlider(0, 255, 43, 1);
  contrastSlider    = createSlider(0, 400, 100, 1);
  dotSpacingSlider    = createSlider(2, 100, 23, 1);
  activeDotSizeSlider = createSlider(1, 60, 41, 1);
  ...
  btnAutomate.mousePressed(() => {
    if (automating) return;
    automating = true;
    automateFrame = 0;
    caGrid = null; caPixels = null;
    caDiff = -1;
    caFrozen = false;
  });
}
...
function draw() {
  ...
  btnAutomate.position(gridX + 8,           gy + 155);
  ...
  if (sourceImage) {
    ...
    if (contoursDirty && window.cvReadyFlag) {
      contoursDirty = false;
      automating = false; caGrid = null; caPixels = null; caFrozen = false;
    ...
    if (layoutMode === 'base') {
      ...
      if (automating) {
        automateFrame++;
        if (automateFrame % 60 === 0) caDiff = -caDiff;
        if (automateFrame >= AUTOMATE_DURATION) {
          automating = false;
          caFrozen = true;
        }
      }
...
function drawDotGrid(gw, gh, contours, cropW, cropH) {
  ...
  let nc = xs.length, nr = ys.length;

  function isActive(xi, yi) {
    if (xi < 0 || xi >= nc || yi < 0 || yi >= nr) return false;
    return maskHitsRect(mask, xs[xi] - half, ys[yi] - half, spacing, spacing);
  }
  function isOutline(xi, yi) {
    if (!isActive(xi, yi)) return false;
    return !isActive(xi - 1, yi) || !isActive(xi + 1, yi) ||
           !isActive(xi, yi - 1) || !isActive(xi, yi + 1);
  }

  // CA initialisation
  if ((automating || caFrozen) && caGrid === null) {
    caW = Math.floor(gw / CA_CELL);
    caH = Math.floor(gh / CA_CELL);
    caGW = gw; caGH = gh;
    caGrid   = new Uint8Array(caW * caH);
    caPixels = new Uint8Array(caW * caH * 4);
    for (let yi = 0; yi < nr; yi++) {
      for (let xi = 0; xi < nc; xi++) {
        if (!isOutline(xi, yi)) continue;
        const col = Math.floor(xs[xi] / CA_CELL);
        const row = Math.floor(ys[yi] / CA_CELL);
        if (col >= 0 && col < caW && row >= 0 && row < caH)
          caGrid[col + row * caW] = 100;
      }
    }
  }

  // CA step
  if (automating && caGrid !== null && caGW === gw && caGH === gh) {
    const next = new Uint8Array(caGrid);
    for (let yi = 0; yi < nr; yi++)
      for (let xi = 0; xi < nc; xi++) {
        if (!isOutline(xi, yi)) continue;
        const col = Math.floor(xs[xi] / CA_CELL);
        const row = Math.floor(ys[yi] / CA_CELL);
        if (col >= 0 && col < caW && row >= 0 && row < caH)
          next[col + row * caW] = 100;
      }
    for (let row = 1; row < caH - 1; row++) {
      for (let col = 1; col < caW - 1; col++) {
        const i = col + row * caW;
        const sum = caGrid[i - caW - 1] + caGrid[i - caW] + caGrid[i - caW + 1]
                  + caGrid[i       - 1]                   + caGrid[i       + 1]
                  + caGrid[i + caW - 1] + caGrid[i + caW] + caGrid[i + caW + 1];
        const avg = Math.floor(sum / 8);
        const nv = (avg + caDiff) & 0xff;
        next[i] = nv;
        const col4 = Math.sin(1 + nv / 4) > Math.sin(caGrid[i] / 4)
          ? 255 : 0;
        caPixels[i * 4]     = col4;
        caPixels[i * 4 + 1] = col4;
        caPixels[i * 4 + 2] = col4;
  ...
  // Render
  noStroke();
  if (automating || caFrozen) {
    if (caGrid !== null && caGW === gw && caGH === gh && caPixels !== null) {
      let img = createImage(caW, caH);
      img.loadPixels();
      img.pixels.set(caPixels);
      img.updatePixels();
      image(img, gridX, gridY, gw, gh);
    }
  } else {
    rectMode(CENTER);
    for (let yi = 0; yi < nr; yi++)
      for (let xi = 0; xi < nc; xi++)
        if (isOutline(xi, yi)) { fill(0); rect(gridX + xs[xi], gridY + ys[yi], activeSize, activeSize); }
  }
  mask.remove();
}
...
function saveImage() {
  ...
  const settings = [
    'Mode: '        + layoutMode,
    'Pad X: '       + nf(padSliderX.value(), 1, 2),
    'Pad Y: '       + nf(padSliderY.value(), 1, 2),
    'Blur: '        + blurSlider.value(),
    'Threshold: '   + threshSlider.value(),
    'Contrast: '    + contrastSlider.value() + '%',
    'Dot spacing: ' + dotSpacingSlider.value() + 'px',
    'Active dot: '  + activeDotSizeSlider.value() + 'px',
    'Image: '       + (showImage ? 'On' : 'Off'),
  ];
  pgUI.textFont('monospace');
  pgUI.textSize(9);
  pgUI.noStroke();
  pgUI.fill(0);
  ...
  if (layoutMode === 'base') {
    ...
    if (r && r.cropPg) {
      ...
      const hasCA = caGrid !== null && caGW === gw && caGH === gh && caPixels !== null;
      if (hasCA) {
        let img = createImage(caW, caH);
        img.loadPixels();
        img.pixels.set(caPixels);
        img.updatePixels();
        pg.imageMode(CORNER);
        pg.image(img, 0, 0, gw, gh);
      } else {
        let spacing = dotSpacingSlider.value();
        let activeSize = activeDotSizeSlider.value();
        let sx = gw / r.cropPg.width, sy = gh / r.cropPg.height;
        ...
        let half = spacing / 2;
        let sxs = [], sys = [];
        for (let x = half; x < gw; x += spacing) sxs.push(x);
        for (let y = half; y < gh; y += spacing) sys.push(y);
        let snc = sxs.length, snr = sys.length;
        function sIsActive(xi, yi) {
          if (xi < 0 || xi >= snc || yi < 0 || yi >= snr) return false;
          return maskHitsRect(maskBuf, sxs[xi] - half, sys[yi] - half, spacing, spacing);
        }
        function sIsOutline(xi, yi) {
          if (!sIsActive(xi, yi)) return false;
          return !sIsActive(xi-1, yi) || !sIsActive(xi+1, yi) ||
                 !sIsActive(xi, yi-1) || !sIsActive(xi, yi+1);
        }
        pg.noStroke(); pg.rectMode(CENTER);
        for (let yi = 0; yi < snr; yi++)
          for (let xi = 0; xi < snc; xi++)
            if (sIsOutline(xi, yi)) { pg.fill(0); pg.rect(sxs[xi], sys[yi], activeSize, activeSize); }
        maskBuf.remove();
      }
    }
  }
