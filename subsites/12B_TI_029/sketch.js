...
function drawTypography() {
  let textNumber = document.getElementById('textNumber').value || '028';
  ...
  for (let col = 0; col < cols; col++) {
    for (let row = 0; row < rows; row++) {
      let cx = patternX() + col * cellW;
      let cy = patternY() + row * cellH;

      let x = (col * cellW | 0) + xOffset;
      let y = (row * cellH | 0) + yOffset;
      let v = applyFormula(x, y) + bitmask;
