...
function setup() {
  ...
  let cnv = createCanvas(canvasWrap.offsetWidth, canvasWrap.offsetHeight);
  ...
  const sliderReadouts = [
    ['padSliderX',       'padXVal',       v => parseFloat(v).toFixed(2)],
    ['padSliderY',       'padYVal',       v => parseFloat(v).toFixed(2)],
    ['blurSlider',       'blurVal',       v => v],
    ['threshSlider',     'threshVal',     v => v],
    ['contrastSlider',   'contrastVal',   v => v],
    ['dotSpacingSlider', 'dotSpacingVal', v => v],
    ['activeDotSizeSlider', 'activeDotVal', v => v],
  ];
  sliderReadouts.forEach(([sliderId, spanId, fmt]) => {
...
function windowResized() {
  let canvasWrap = document.querySelector('.canvas-wrap');
  resizeCanvas(canvasWrap.offsetWidth, canvasWrap.offsetHeight);
  contoursDirty = true;
}
...
function draw() {
  ...
  let pv = sliderVal('padSliderX'), pvY = sliderVal('padSliderY'), bv = sliderVal('blurSlider'),
      tv = sliderVal('threshSlider'), cv = sliderVal('contrastSlider');
...
function drawDotGrid(gw, gh, contours, cropW, cropH) {
  ...
  //EDIT HERE$$
  angleMode(DEGREES);stroke(0); strokeWeight(1); noFill();//fill(255);  //rectMode(CENTER);
  for (let yi = 0; yi < nr; yi++)
    for (let xi = 0; xi < nc; xi++)
      if (isOutline(xi, yi)) {
        ...
        //shearX(45);
        rotate(25);
        //rect(0, 0, activeSize, activeSize);//rect(0, 0, cellW*sin(yi), rows*cellH - (gridY + ys[yi]));
        line(10, 0, 0, activeSize*5);//line(0, 0, 0, rows*cellH - (gridY + ys[yi])); //line(0, 0, activeSize, activeSize);
        pop();
      }
  mask.remove();
}
...
function saveImage() {
  ...
  if (layoutMode === 'base') {
    ...
    if (r && r.cropPg) {
      ...
      //EDIT HERE$$
      pg.stroke(0); pg.strokeWeight(1); pg.angleMode(DEGREES); pg.noFill(); //; //pg.rectMode(CENTER);
      for (let yi = 0; yi < snr; yi++)
        for (let xi = 0; xi < snc; xi++)
          if (sIsOutline(xi, yi)) {
            ...
            //pg.shearX(45);
            pg.rotate(25);
            //pg.rect(0, 0, activeSize, activeSize);
            pg.line(10, 0, 0, activeSize*5)//pg.rect(0, 0, cellW*sin(yi), rows*cellH - sys[yi]);
            pg.pop();
          }
      maskBuf.remove();
    }
