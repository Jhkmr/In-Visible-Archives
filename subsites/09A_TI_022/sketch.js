...
function draw() {
  ...
  if (sourceImage) {
    ...
    if (contoursDirty && window.cvReadyFlag) {
      contoursDirty = false;
      automating = false; caGrid = null; caFrozen = false;
...
function drawDotGrid(gw, gh, contours, cropW, cropH) {
  ...
  if ((automating || caFrozen) && caGrid === null) {
    ...
    caGridRows = nr;
    caSpacing  = spacing;
    caGrid = new Float32Array(nc * nr);
    for (let yi = 0; yi < nr; yi++) {
      for (let xi = 0; xi < nc; xi++) {
        if (isOutline(xi, yi)) caGrid[xi + yi * nc] = 1.0;
      }
    }
  }

  // CA step: diffuse outward
  if (automating && caGrid !== null && caSpacing === spacing &&
      caGridCols === nc && caGridRows === nr) {
    const decay = 0.96;
    const next = new Float32Array(caGrid);
    for (let yi = 0; yi < nr; yi++) {
      for (let xi = 0; xi < nc; xi++) {
        const i = xi + yi * nc;
        if (isOutline(xi, yi)) { next[i] = 1.0; continue; }
        let sum = 0, count = 0;
        if (xi > 0)      { sum += caGrid[(xi-1) + yi * nc]; count++; }
        if (xi < nc - 1) { sum += caGrid[(xi+1) + yi * nc]; count++; }
        if (yi > 0)      { sum += caGrid[xi + (yi-1) * nc]; count++; }
        if (yi < nr - 1) { sum += caGrid[xi + (yi+1) * nc]; count++; }
        const avg = count > 0 ? sum / count : 0;
        next[i] = Math.max(caGrid[i], avg) * decay;
      }
    }
    caGrid = next;
  }
  ...
  for (let yi = 0; yi < nr; yi++) {
    for (let xi = 0; xi < nc; xi++) {
      if (automating || caFrozen) {
        const val = (caGrid !== null && caGridCols === nc && caGridRows === nr &&
                     caSpacing === spacing)
                    ? caGrid[xi + yi * nc] : 0;
        if (val < 0.01) continue;
        const d = val * activeSize;
        fill(0);
        rect(gridX + xs[xi], gridY + ys[yi], d, d);
      } else {
        if (!isOutline(xi, yi)) continue;
        fill(0);
        rect(gridX + xs[xi], gridY + ys[yi], activeSize, activeSize);
      }
    }
  }
  mask.remove();
...
function saveImage() {
  ...
  if (layoutMode === 'base') {
    ...
    if (r && r.cropPg) {
      ...
      const hasCA = caGrid !== null && caGridCols === snc && caGridRows === snr && caSpacing === spacing;
      pg.fill(0); pg.noStroke(); pg.rectMode(CENTER);
      for (let yi = 0; yi < snr; yi++) {
        for (let xi = 0; xi < snc; xi++) {
          if (hasCA) {
            const val = caGrid[xi + yi * snc];
            if (val < 0.01) continue;
            pg.rect(sxs[xi], sys[yi], val * activeSize, val * activeSize);
          } else {
            if (!sIsOutline(xi, yi)) continue;
            pg.rect(sxs[xi], sys[yi], activeSize, activeSize);
          }
        }
      }
      maskBuf.remove();
