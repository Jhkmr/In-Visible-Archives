...
function setup() {
  ...
  dotSpacingSlider    = createSlider(5, 100, 32, 1);
  activeDotSizeSlider = createSlider(1, 60, 60, 1);
...
function drawDotGrid(gw, gh, contours, cropW, cropH) {
  ...
  for (let x = half; x < gw; x += spacing) {
    for (let y = half; y < gh; y += spacing) {
      ...
      let d = active ? activeSize : 0;
      rect(gridX + x - d/2, gridY + y - d/2, d, d);
    }
  }
  mask.remove();
}
    ...
    if (r && r.cropPg) {
      ...
      for (let x = half; x < gw; x += spacing) {
        for (let y = half; y < gh; y += spacing) {
          ...
          let d = active ? activeSize : 0;
          pg.rect(gridX + x - d/2, gridY + y - d/2, d, d);
        }
      }
      maskBuf.remove();
    }
